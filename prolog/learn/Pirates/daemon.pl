#!/home/paul/.nix-profile/bin/swipl

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
ClioPatria is prepared to be combined with the SWI-Prolog library
library(http/http_unix_daemon).  ClioPatria is started as a daemon
using
...
distribution contains init scripts in /packages/http/examples/,
which can be used as a skeleton for managing the server from /etc/init.d.
To do so, follow these steps:

  1. copy your init-script to /etc/init.d/<server>.  This
